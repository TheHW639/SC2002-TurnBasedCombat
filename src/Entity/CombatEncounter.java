...
public class CombatEncounter {
    ...
    public void endRound() {
        ...
        int finalPotionCount = 0, finalSmokeBombCount = 0, finalPowerStoneCount = 0;
        for (item.Item item : player.getBag().getItems()) {
            if (item instanceof item.Potion) {
                finalPotionCount = item.getQuantity();
            } else if (item instanceof item.SmokeBomb) {
                finalSmokeBombCount = item.getQuantity();
            } else if (item instanceof item.PowerStone) {
                finalPowerStoneCount = item.getQuantity();
            }
        }
        ...
        if (currentEnemies.isEmpty()) {
            if (difficultyWaves.hasNextWave()) {
                ...
                battleEnded = true;
                display.showVictory(player, roundCounter, 
                    finalPotionCount, finalSmokeBombCount, finalPowerStoneCount);
                return;
            }
        }
        ...
        if (player.getHealth().isDead()) {
            battleEnded = true;
            display.showGameOver(player, roundCounter, 
                finalPotionCount, finalSmokeBombCount, finalPowerStoneCount);
            return;
        }
    ...
    public void checkGameState() {
        if (player.getHealth().isDead()) {
            ...
            display.showGameOver(player, roundCounter, potionCount, smokeBombCount, powerStoneCount);
        }
        ...
        if (currentEnemies.isEmpty() && !difficultyWaves.hasNextWave()) {
            ...
            display.showVictory(player, roundCounter, potionCount, smokeBombCount, powerStoneCount);
        }
    }
    ...
    private void displayStats() {
        ...
        for (item.Item item : player.getBag().getItems()) {
            if (item instanceof item.Potion) {
                potionCount = item.getQuantity();
            } else if (item instanceof item.SmokeBomb) {
                smokeBombCount = item.getQuantity();
            } else if (item instanceof item.PowerStone) {
                powerStoneCount = item.getQuantity();
            }
        }
